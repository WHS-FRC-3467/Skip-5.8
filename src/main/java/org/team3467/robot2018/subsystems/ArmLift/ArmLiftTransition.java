...
public class ArmLiftTransition extends Command {
    ...
    protected void initialize() {
    	...
    	ArmLift.eArmLiftState currentState = Robot.armLift.getState();
       	...
       	if (currentState == ArmLift.eArmLiftState.Unknown) {
    		// State Unknown; Need to just go to HiLo first
    ...
    protected void execute() {
    	// Check isFinished flag and return if set
    	if (m_finished)
    		return;
    	...
    	if (!Robot.armLift.moveMagically(m_leftTarget, m_rightTarget))
    		m_finished = true;
    ...
    protected boolean isFinished() {
    	...
    	// check isFinished flag and if set, return TRUE immediately
    	if (m_finished)
    		return true;
